    ...
    ctx.app.event("channel_id_changed", async ({ event, body }) => {
        try {
            ...
            if (migration.migrated) {
                ...
                ctx.runtime.log?.(warn(`[slack] Channel config already exists for ${newChannelId}; leaving ${oldChannelId} unchanged`));
            }
            else {
                ctx.runtime.log?.(warn(`[slack] No config found for old channel ID ${oldChannelId}; migration logged only`));
            }
        }
        catch (err) {
