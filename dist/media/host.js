...
export async function ensureMediaHosted(source, opts = {}) {
    ...
    if (needsServerStart && !opts.startServer) {
        ...
        throw new Error(`Media hosting requires the webhook/Funnel server. Start \`${formatCliCommand("openclaw webhook")}\`/\`${formatCliCommand("openclaw up")}\` or re-run with --serve-media.`);
    }
    if (needsServerStart && opts.startServer) {
        if (!mediaServer) {
