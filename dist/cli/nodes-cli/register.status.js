...
export function registerNodesStatusCommands(nodes) {
    nodesCallOpts(nodes
        .command("status")
        ...
        await runNodesCommand("status", async () => {
            ...
            const lastConnectedById = sinceMs !== undefined
                ? new Map(parsePairingList(await callGatewayCli("node.pair.list", opts, {})).paired.map((entry) => [entry.nodeId, entry]))
                : null;
            const filtered = nodes.filter((n) => {
                ...
                if (sinceMs !== undefined) {
                    ...
                    const lastConnectedAtMs = typeof paired?.lastConnectedAtMs === "number"
            ...
            const rows = filtered.map((n) => {
                ...
                const since = typeof n.connectedAtMs === "number"
    ...
    nodesCallOpts(nodes
        ...
        await runNodesCommand("list", async () => {
            ...
            const connectedById = hasFilters
                ? new Map(parseNodeList(await callGatewayCli("node.list", opts, {})).map((node) => [
                    node.nodeId,
                    node,
                ]))
                : null;
            const filteredPaired = paired.filter((node) => {
                ...
                if (sinceMs !== undefined) {
                    const live = connectedById?.get(node.nodeId);
                    const lastConnectedAtMs = typeof node.lastConnectedAtMs === "number"
            ...
            if (opts.json) {
                defaultRuntime.log(JSON.stringify({ pending: pendingRows, paired: filteredPaired }, null, 2));
                return;
            }
            ...
            if (filteredPaired.length > 0) {
                const pairedRows = filteredPaired.map((n) => {
                    ...
                    const lastConnectedAtMs = typeof n.lastConnectedAtMs === "number"
                        ? n.lastConnectedAtMs
                        : typeof live?.connectedAtMs === "number"
                            ? live.connectedAtMs
                            : undefined;
