    ...
    addGatewayClientOptions(cron
        ...
        try {
            const schedule = (() => {
                ...
                const chosen = [Boolean(at), Boolean(every), Boolean(idle), Boolean(cronExpr)].filter(Boolean).length;
                if (chosen !== 1) {
                ...
                if (idle) {
                    ...
                    const stopOn = stopOnRaw
                        ? stopOnRaw
                            ...
                            .filter((s) => s === "user" || s === "agent")
                        : undefined;
                    return { kind: "idle", timeoutMs, resetOn, stopOn };
                }
                return {
                    kind: "cron",
                    expr: cronExpr,
            ...
            if (typeof sessionTarget === "object" &&
                sessionTarget.key &&
