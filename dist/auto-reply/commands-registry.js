...
export function normalizeCommandBody(raw, options) {
    const trimmed = raw.trim();
...
export function getCommandDetection(_cfg) {
    const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw, cfg) {
...
export function resolveTextCommand(raw, cfg) {
    const trimmed = normalizeCommandBody(raw).trim();
