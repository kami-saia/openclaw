...
async function persistSessionCompactionCheckpoint(
  ...
  const updatedEntry = await patchSessionEntryCore(
    ...
    async (existing) => {
      ...
      stored = true;
      // FORK: compaction may rotate the active transcript. Keep the top-level
      // routing identity aligned with the successor transcript, otherwise the
      // next inbound message can target an orphaned transcript that cleanup removed.
      // Upstream retired `SessionEntry.sessionFile` (stripRetiredSessionEntryLocators),
      // so sessionId rotation is now the only durable rotation signal.
      const rotated = params.sessionId !== existing.sessionId;
      return {
        ...(rotated ? { sessionId: params.sessionId } : {}),
