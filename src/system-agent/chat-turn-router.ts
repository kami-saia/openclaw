...
type ChatTurnRouterOptions = {
  ...
  /** FORK: bypass the operator approval handshake entirely (see chat-engine.ts). */
...
export class ChatTurnRouter {
  ...
  async resolveTurn(
    ...
    if (
      this.options.operatorApprovalOnly &&
      !this.options.trustDelegatedOperatorApproval &&
    ...
    const intent =
      this.options.operatorApprovalOnly && !this.options.trustDelegatedOperatorApproval
        ? "other"
