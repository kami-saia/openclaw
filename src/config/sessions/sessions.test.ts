...
describe("resolveSessionResetPolicy", () => {
  describe("backward compatibility: resetByType.dm -> direct", () => {
    it("does not use dm fallback for group/thread types", () => {
      const sessionCfg = {
        // FORK: DEFAULT_RESET_MODE is "never"; set explicit daily default so this
        // upstream backward-compat test exercises the dm-fallback mechanics.
  ...
  it("defaults to daily resets at 4am local time", () => {
    const policy = resolveSessionResetPolicy({
      // FORK: DEFAULT_RESET_MODE is "never"; pin explicit daily to assert the
      // 4am daily mechanics upstream expects by default.
      sessionCfg: { reset: { mode: "daily" as const } } as unknown as SessionConfig,
      resetType: "direct",
    });
