...
describe("applyAgentAutoCompactionGuard", () => {
  ...
  // FORK: agent-mode gating. Agent owns compaction via compact tool +
  // pressure signal, so Pi's internal auto-compaction must be disabled.
  it("disables Pi auto-compaction in fork agent mode", () => {
    const setCompactionEnabled = vi.fn();
    const settingsManager = {
