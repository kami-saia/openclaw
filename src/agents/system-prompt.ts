...
// FORK: soul files are emitted by buildSoulSection above, so they are excluded
// here and never get the "SOUL.md: persona/tone" bullet upstream adds.
...
function buildRuntimeLine(
  ...
  return `Runtime: ${[
    ...
    // FORK: surfaced only when the provider-reported served model DIVERGES from
    // the requested one, making a silent provider swap visible without a tool call.
