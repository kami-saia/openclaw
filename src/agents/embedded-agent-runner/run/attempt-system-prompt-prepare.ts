...
export async function prepareEmbeddedAttemptSystemPrompt(params: {
  ...
  if (attempt.operation === "settled-tool-finalization") {
    ...
    return {
      // FORK: finalization never re-renders; keep the shape uniform so callers
      // do not have to narrow an optional renderer.
  ...
  // is known from the restored in-memory transcript. The runtimeInfo object is
  // mutated in place with servedModel; no new IO/async.
