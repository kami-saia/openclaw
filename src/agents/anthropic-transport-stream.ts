...
/**
 * FORK: Reads an explicit per-model thinking-shape override from config
 * (`params.thinkingApi`). Returns "adaptive" | "enabled" | undefined.
 * Config wins over the upstream id heuristic so new Claude releases (and
 * 3rd-party anthropic-compatible endpoints like Kimi that need "enabled")
 * work without a code change. See TOOLS.md "Anthropic Thinking Shape".
 */
    ...
    if (msg.role === "assistant") {
      ...
      for (const block of msg.content) {
        ...
        if (block.type === "thinking") {
          ...
          // FORK: drop Copilot thinking signatures on replay (Copilot 400 fix).
          // See TOOLS.md "Copilot thinking-signature replay".
