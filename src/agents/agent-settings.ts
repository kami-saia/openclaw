...
export function resolveEffectiveCompactionMode(cfg?: OpenClawConfig): AgentCompactionMode {
  ...
  // Fork: agent mode is preserved (not promoted to safeguard).
  if (compaction?.mode === "agent") {
    return "agent";
  }
...
export function shouldDisableAgentAutoCompaction(params: {
  ...
  return (
    ...
    // Fork: in agent-controlled compaction mode, the agent owns compaction
    // via the `compact` tool + pressure signal. Disable Pi's internal
    // auto-trigger so the safeguard extension does not run system-style
    // summarization behind the agent's back.
    params.compactionMode === "agent" ||
