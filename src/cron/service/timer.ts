...
async function executeJobCore(
  ...
  const isSessionTarget =
    typeof job.sessionTarget === "object" && job.sessionTarget.kind === "session";

  if (job.sessionTarget === "main" || isSessionTarget) {
    const targetSessionKey = isSessionTarget
      ? (job.sessionTarget as { key: string }).key
      : undefined;
    ...
    if (job.wakeMode === "now" && state.deps.runHeartbeatOnce) {
      ...
      const heartbeatConfig = targetSessionKey ? { session: targetSessionKey } : undefined;
