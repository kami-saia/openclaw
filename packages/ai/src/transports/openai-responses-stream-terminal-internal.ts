...
export function createResponsesTerminalController(params: {
  ...
  const appendText = (item: ResponseOutputMessage, contentIndex?: number): number | undefined => {
    ...
    if (collapse.kind === "extend" && previous) {
      ...
      // FORK: a pure repeat adds no text; re-ending the block would deliver the
  ...
  const recoverTerminalOutput = (
    ...
    for (const [terminalIndex, item] of items.entries()) {
      if (item.type === "message") {
        ...
        // FORK (#eyrie-triple): items already projected by the streaming path are
        // replayed verbatim in the terminal snapshot; they are not a new boundary,
        // so clearing the collapse candidate here would strand a duplicate
        // final-answer message that only shows up in the snapshot.
        if (params.outputs.get(item, terminalIndex) === undefined) {
